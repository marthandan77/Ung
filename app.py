...
def new_engine(position_qty: int, average_cost: float, minimum_profit: float) -> DecisionEngineV8RTIS:
    return DecisionEngineV8RTIS(
        EngineConfig(
...
def apply_bar(bar: MarketBar, engine: DecisionEngineV8RTIS, db: SQLiteJournal) -> Decision:
...
with right:
    ...
    if st.button("Fetch Latest", use_container_width=True):
        try:
            ...
            model_seed = max(engine.config.hmm_min_samples + 40, engine.config.garch_min_returns + 40)
            if seeded > 0 or len(engine.feature_rows) < engine.config.hmm_min_samples:
                for seed_bar in client.recent_bars(limit=max(300, seeded + 20, model_seed)):
...
if decision is None:
    ...
    c4.metric("HMM", s.model_status)
    c5.metric("GARCH", s.garch_status)
    ...
    rows = [
        ...
        ("Regime state", s.regime_state_id),
        ("Regime probabilities", json.dumps(s.regime_probabilities, sort_keys=True) if s.regime_probabilities else "NOT_READY"),
        ("Markov state", s.markov_current_state),
        ("Markov next", json.dumps(s.markov_next_state_probabilities, sort_keys=True) if s.markov_next_state_probabilities else "NOT_READY"),
        ("Markov warning", s.transition_warning),
        ("GARCH status", s.garch_status),
        ("Vol forecast", f"{s.next_period_volatility_forecast:.5f}"),
        ("Last HMM fit", s.last_fit_time),
        ("HE / RP / MQI", f"{s.he:.1f} / {s.rp:.1f} / {s.mqi:.1f}"),
