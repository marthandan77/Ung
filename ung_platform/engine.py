...
class DecisionEngineV7Lite:
    ...
    def __init__(self, config: EngineConfig | None = None):
        ...
        self.feature_rows: deque[tuple[datetime, list[float]]] = deque(maxlen=2500)
        self.regime_sequence: deque[int] = deque(maxlen=1200)
    ...
    def _fit_hmm_if_ready(self, timestamp: datetime) -> None:
        ...
        try:
            ...
            if not np.isfinite(matrix).all():
                ...
                return
            components = min(
                max(2, int(self.config.hmm_components)),
                max(2, len(self.feature_rows) // 60),
            )
            model = GaussianHMM(
    ...
    def _apply_real_markov(self, s: FeatureSnapshot) -> None:
        if s.model_status != "READY" or len(self.regime_sequence) < self.config.markov_min_transitions:
            ...
            s.regime_persistence_probability = None
        ...
        for current, row in counts.items():
            ...
            matrix[current] = {nxt: count / total for nxt, count in row.items()} if total else {}
    ...
    def _apply_real_garch_or_ewma(self, s: FeatureSnapshot) -> None:
        ...
        if (
            arch_model is not None
            and np is not None
            and len(self.returns) >= self.config.garch_min_returns
            and self.garch_status == "READY"
            and self.bar_count % max(1, self.config.garch_fit_interval_bars) != 0
        ):
...
DecisionEngineV8RTIS = DecisionEngineV7Lite
